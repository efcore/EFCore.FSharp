...
type ModelCodeGeneratorTestBase() =
    ...
    member this.Test (buildModel : ModelBuilder -> unit) (options : ModelCodeGenerationOptions) (assertScaffold : ScaffoldedModel -> unit) (assertModel : IModel -> unit) =
        ...
        let designServices = ServiceCollection() :> IServiceCollection
        // Add F# overrides
        ...
        let modelBuilder = SqlServerTestHelpers.Instance.CreateConventionBuilder(customServices = designServices)
        ...
        let model = modelBuilder.FinalizeModel(designTime = true, skipValidation = true)
