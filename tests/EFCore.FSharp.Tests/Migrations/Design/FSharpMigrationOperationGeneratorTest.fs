...
type GeometryValueCoverter<'geometry when 'geometry :> Geometry>(reader : SqlServerBytesReader, writer : SqlServerBytesWriter) =
...
type SqlServerGeometryTypeMapping<'geometry when 'geometry :> Geometry> (geometryServices:NtsGeometryServices, storeType: string) =
    inherit RelationalGeometryTypeMapping<'geometry, SqlBytes>((Helpers.createConverter geometryServices storeType), storeType)

    let isGeography =
        String.Equals(storeType, "geography", StringComparison.OrdinalIgnoreCase)

    let getSqlBytes = typeof<SqlDataReader>.GetRuntimeMethod("GetSqlBytes", [| typeof<int> |])
    ...
    let createSqlDbTypeAccessor paramType =
        ...
        Expression.Lambda<Action<DbParameter, SqlDbType>>(
            Expression.Call(
    ...
    let createUdtTypeNameAccessor(paramType) =
        ...
        Expression.Lambda<Action<DbParameter, string>>(
            Expression.Call(
                Expression.Convert(paramParam, paramType),
    ...
    override __.GenerateNonNullSqlLiteral(value) =
        ...
        let defaultSrid = (geometry = (Point.Empty :> Geometry)) || (geometry.SRID = (if isGeography then 4326 else 0))
    ...
    override __.AsText value =
        (value :?> Geometry).AsText()

    override __.GetSrid value =
        (value :?> Geometry).SRID
...
type SqlServerNetTopologySuiteTypeMappingSourcePlugin (geometryServices) =
    ...
    interface IRelationalTypeMappingSourcePlugin with
        member __.FindMapping(mappingInfo) =
            ...
            if typeof<Geometry>.IsAssignableFrom(clrType) || (notNull storeTypeName && spatialStoresTypes.Contains(storeTypeName)) then
...
module FSharpMigrationOperationGeneratorTest =
    ...
    let Test<'a when 'a :> MigrationOperation> (operation:'a) (expectedCode:string) (``assert``:('a -> unit)) =
        ...
        builder.IncrementIndent() |> ignore
        (generator :> ICSharpMigrationOperationGenerator).Generate("mb", [| operation |], builder)
        ...
        let factoryType = assembly.GetTypes() |> Seq.find(fun m -> m.Name = "OperationsFactory")
    ...
    let FSharpMigrationOperationGeneratorTest =
        testList "FSharpMigrationOperationGeneratorTest" [
            ...
            test "Generate separates operations by a blank line" {
                ...
                generator.Generate(
                    "mb",
                    [| SqlOperation ( Sql = "-- Don't stand so" ); SqlOperation ( Sql = "-- close to me" ) |],
                    builder)

                let expected = "mb.Sql(\"-- Don't stand so\") |> ignore" + _eol + _eol + "mb.Sql(\"-- close to me\") |> ignore" + _eol + _eol
            ...
            test "CreateTableOperation optional args" {
                ...
                op.Columns.Add <| AddColumnOperation(Name = "Id", Table = "MyTable", ClrType = typeof<Guid>)
