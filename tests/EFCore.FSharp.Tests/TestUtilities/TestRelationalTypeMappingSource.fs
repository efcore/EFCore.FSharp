...
type TestRelationalTypeMappingSource(dependencies, relationalDependencies) =
    ...
    let _defaultDateTimeMapping =
        DateTimeTypeMapping("default_datetime_mapping", dbType = Nullable<DbType>(DbType.DateTime2))
        :> RelationalTypeMapping

    let _defaultDoubleMapping =
        DoubleTypeMapping("default_double_mapping") :> RelationalTypeMapping

    let _defaultDateTimeOffsetMapping =
        DateTimeOffsetTypeMapping("default_datetimeoffset_mapping") :> RelationalTypeMapping

    let _defaultFloatMapping =
        FloatTypeMapping("default_float_mapping") :> RelationalTypeMapping

    let _defaultGuidMapping =
        GuidTypeMapping("default_guid_mapping") :> RelationalTypeMapping

    let _defaultTimeSpanMapping =
