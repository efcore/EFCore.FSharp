...
type ImplementationType = { Type: Type; ImplementationType: Type }
...
module TestServiceFactory =
    ...
    let private getImplementationTypes (serviceType: Type) =
        ...
        if serviceType.IsInterface then
            ...
            let implementationTypes =
                st.Assembly.GetTypes()
                |> Seq.filter (fun t ->
                    st.IsAssignableFrom(t)
                    && (not t.IsAbstract)
                )
            ...
            | None ->
                ...
                then
                    let msg =
                        sprintf
                            "Cannot use 'TestServiceFactory' for '%s': no single implementation type in same assembly."
                            (serviceType.Name)
    ...
    let rec private addType
        (serviceCollection: ServiceCollection)
        (serviceType: Type)
        (specialCases: Implementation list)
        =
        let implementation =
            specialCases
