...
module WebServer =
    ...
    let useAsync (middlware: HttpContext -> (unit -> Async<unit>) -> Async<unit>) (app: IApplicationBuilder) =
        app.Use
            (fun env (next: Func<Threading.Tasks.Task>) ->
            ...
            if httpContext.WebSockets.IsWebSocketRequest then
                ...
                use d =
                    ...
                    |> Observable.subscribe
                        (fun m ->
                            let segment =
                                ArraySegment<byte>(m |> Text.Encoding.UTF8.GetBytes)
    ...
    let startWebserver (refreshWebpageEvent: Event<string>) docsDir (url: string) =
        WebHostBuilder()
            ...
            .Configure(fun app ->
                let opts =
                    StaticFileOptions(FileProvider = new PhysicalFileProvider(docsDir))
    ...
    let openBrowser url =
        let waitForExit (proc: Process) =
            ...
            if proc.ExitCode <> 0 then
                eprintf "opening browser failed, open your browser and navigate to url to see the docs site."

        try
            let psi =
                ProcessStartInfo(FileName = url, UseShellExecute = true)
        ...
        | e ->
            ...
            if RuntimeInformation.IsOSPlatform(OSPlatform.Windows) then
                ...
                let psi =
                    ProcessStartInfo("cmd", (sprintf "/c %s" url), CreateNoWindow = true)
