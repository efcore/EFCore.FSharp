...
let repoSourceLink (m: Member) =
    seq {
        if m.Details.FormatSourceLocation
           |> String.IsNullOrEmpty
           |> not then
            yield
                a [ Href m.Details.FormatSourceLocation
                    Class "float-right"
                    HTMLAttr.Custom("aria-label", "View source on GitHub") ] [
                    yield i [ Class "fab fa-github text-dark" ] []
                ]
    }
...
let normalize (content: string) = content |> replaceh2withh5
...
let commentBlock (c: Comment) =
    ...
    let renderSection (s: KeyValuePair<string, string>) : Fable.React.ReactElement list =
        match s with
        | EmptyDefaultBlock -> []
        | NonEmptyDefaultBlock content ->
            [ div [ Class "comment-block" ] [
                  RawText(normalize content)
              ] ]
        | Section (name, content) ->
            [ h5 [] [ str name ] // h2 is obnoxiously large for this context, go with the smaller h5
              RawText(normalize content) ]

    c.Sections |> List.collect renderSection

let compiledName (m: Member) =
    seq {
        if m.Details.FormatCompiledName
           |> String.IsNullOrEmpty
           |> not then
            yield
                p [] [
                    ...
                    code [] [
                        str m.Details.FormatCompiledName
                    ]
                ]
    }
