          ...
          div [ Class "container-fluid py-3" ] [
              yield
                  div [ Class "row" ] [
                      yield
                          div [ Class "col-12" ] [
                              yield h1 [] [ str moduleInfo.Module.Name ]
                              ...
                              yield
                                  dl [] [
                                      if moduleInfo.ParentModule.IsSome then
                                          yield dt [] [ str "Parent Module" ]

                                          yield
                                              dd [] [
                                                  a [ Href(sprintf "%s.html" moduleInfo.ParentModule.Value.UrlName) ] [
                                                      str moduleInfo.ParentModule.Value.Name
                                                  ]
                                              ]
                                      if moduleInfo.Module.Attributes |> Seq.isEmpty |> not then
                                          yield dt [] [ str "Attributes" ]
                              ...
                              yield
                                  div [ Class "xmldoc" ] [
                                      for sec in comment.Sections do
                                          if byCategory
                                             |> Seq.exists (fun g -> g.GroupKey = sec.Key)
                                             |> not then
                                              if sec.Key <> "<default>" then
                                                  yield h2 [] [ RawText sec.Key ]
                                  ...
                                  yield
                                      ul [] [
                                          for g in byCategory do
                                              yield
                                                  li [] [
                                                      a [ Href(g.Index.ToString() |> sprintf "#section%s") ] [
                                                          str g.Name
                                                      ]
                                                  ]
                                      ]

                              if (nestTypes |> Seq.length)
                                 + (nestModules |> Seq.length) > 0 then
                                  yield h2 [] [ str "Nested types and modules" ]
                              ...
                              for g in byCategory do
                                  ...
                                  yield!
                                      partMembers
                                          ...
                                          "Function or value"
                                          (g.Members
                                           |> Seq.filter (fun m -> m.Kind = MemberKind.ValueOrFunction))
                                  ...
                                  yield!
                                      partMembers
                                          ...
                                          "Type extension"
                                          (g.Members
                                           |> Seq.filter (fun m -> m.Kind = MemberKind.TypeExtension))
                                  ...
                                  yield!
                                      partMembers
                                          ...
                                          "Active pattern"
                                          (g.Members
                                           |> Seq.filter (fun m -> m.Kind = MemberKind.ActivePattern))
                          ]
                  ]
          ] ]
