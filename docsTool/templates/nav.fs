...
let navItem link inner =
    li [ Class "nav-item" ] [
        a [ Class "nav-link"; Href link ] inner
    ]

let navItemText text link = navItem link [ normalizeStr text ]
...
let dropDownNavMenu text items =
    li [ Class "nav-item dropdown" ] [
        ...
        ul
            [ HTMLAttr.Custom("aria-labelledby", "dropdownMenu1")
...
let navTreeFromPaths (rootPath: IO.DirectoryInfo) (files: IO.FileInfo list) =
    ...
    and addHeadPath subFilePath (part: string) remainingParts (nodes: NavTree list) =
        ...
        | [] ->
            ...
            |> List.singleton
        | Folder (title, subnodes) :: nodes when title = part ->
            Folder(title, addPath subFilePath remainingParts subnodes)
            :: nodes
        | hn :: tn ->
            hn
    ...
    ||> List.fold
            (fun state file ->
                let subFilePath =
                    file.FullName.Replace(rootPath.FullName, "")

                let pathParts =
                    subFilePath.Split(IO.Path.DirectorySeparatorChar, StringSplitOptions.RemoveEmptyEntries)
                    |> Array.toList
    ...
    nav [ Class "navbar navbar-expand-md sticky-top navbar-dark bg-dark" ] [
        ...
        div [ Class "collapse navbar-collapse"
            ...
            ul [ Class "navbar-nav mr-auto" ] [
                yield!
                    navTreeFromPaths navCfg.TopLevelNav.DocsRoot navCfg.TopLevelNav.DocsPages
                    |> sortNavTree
                    |> generateNavMenus navCfg.SiteBaseUrl
            ]
            ul [ Class "navbar-nav" ] [
                ...
                navItemIconOnly
                    (string navCfg.GitHubRepoUrl)
                    (sprintf "%s Repository on Github" navCfg.ProjectName)
